...
async function display_meals() {
  ...
  for (let i = 0; i < res.meals.length; i++) {
    cartona += `
      ...
      <div class="col-md-3">
                ...
                <div class="own_col">
                    <div class="own_card  position-relative rounded-3  mt-2 text-white p-5   ">

                        <i class="fa-solid fa-house-laptop fa-2xl mb-5"></i>
                            <p class="fw-bold fs-3 ">${res.meals[i].strArea}</p>
  ...
  $(".own_card").click(function (e) {
    let food_name = e.delegateTarget.innerText;
    let cartona = "";
    console.log(food_name);
    // let mahmoud = "corba";
    async function getapi() {
      let api = await fetch(
        `https://www.themealdb.com/api/json/v1/1/filter.php?a=${food_name}`
      );
      let res = await api.json();
      console.log(res);
