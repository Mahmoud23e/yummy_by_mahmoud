...
async function display_ingrediants() {
  let cartona = "";
  let api = await fetch(
  ...
  for (let i = 0; i < 24; i++) {
    cartona += `
         ...
         <div class="col-md-3">
                    ...
                    <div class="own_col">
                        <div class="own_card  position-relative rounded-3  mt-2 text-white   ">

                            <i class="fa-solid fa-drumstick-bite"></i>
                            <p class="fw-bold fs-1  ">${res.meals[i].strIngredient}</p>
                            <p>${res.meals[i].strDescription}</p>
  ...
  $(".own_card").click(function (e) {
    let food_name = e.delegateTarget.childNodes[3].innerHTML;
    let cartona = "";
    console.log(food_name);
    // let mahmoud = "corba";
    async function getapi() {
      let api = await fetch(
        `https://www.themealdb.com/api/json/v1/1/filter.php?i=${food_name}`
      );
      let res = await api.json();
      console.log(res);
      ...
      $(".own_card").click(function (e) {
        // document.querySelector(".row.g-3").innerHTML = cartona;
        // console.log(cartona);
        let food_name = e.delegateTarget.children[1].innerText;
        console.log(food_name);
        // let mahmoud = "corba";
        async function getapi() {
          let api = await fetch(
            `https://themealdb.com/api/json/v1/1/search.php?s=${food_name}`
          );
          let res = await api.json();
          console.log(res);
