...
const bars = $(".bars");
const input = $("._input");
const search_input_by_fr_le = $(".input_by_fr_le");
...
search_input_by_fr_le.on("keyup", function (e) {
  search_api_by_letter(e.target.value);
});

async function search_api_by_letter(query) {
  let cartona = "";
  let api = await fetch(
...
async function serch_meals(searched_value) {
  let cartona = "";
  let api = await fetch(
    `https://www.themealdb.com/api/json/v1/1/search.php?s=${searched_value}`
  );
  let res = await api.json();
  //   console.log(res);
  for (let i = 0; i < 20; i++) {
